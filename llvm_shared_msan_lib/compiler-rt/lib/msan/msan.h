...
namespace __msan {
SANITIZER_INTERFACE_ATTRIBUTE extern int msan_inited;
SANITIZER_INTERFACE_ATTRIBUTE extern bool msan_init_is_running;
SANITIZER_INTERFACE_ATTRIBUTE extern int msan_report_count;

SANITIZER_INTERFACE_ATTRIBUTE bool ProtectRange(uptr beg, uptr end);
SANITIZER_INTERFACE_ATTRIBUTE bool InitShadow(bool init_origins);
SANITIZER_INTERFACE_ATTRIBUTE char *GetProcSelfMaps();
SANITIZER_INTERFACE_ATTRIBUTE void InitializeInterceptors();

SANITIZER_INTERFACE_ATTRIBUTE void MsanAllocatorInit();
SANITIZER_INTERFACE_ATTRIBUTE void MsanDeallocate(StackTrace *stack, void *ptr);

SANITIZER_INTERFACE_ATTRIBUTE void *msan_malloc(uptr size, StackTrace *stack);
SANITIZER_INTERFACE_ATTRIBUTE void *msan_calloc(uptr nmemb, uptr size, StackTrace *stack);
SANITIZER_INTERFACE_ATTRIBUTE void *msan_realloc(void *ptr, uptr size, StackTrace *stack);
SANITIZER_INTERFACE_ATTRIBUTE void *msan_reallocarray(void *ptr, uptr nmemb, uptr size, StackTrace *stack);
SANITIZER_INTERFACE_ATTRIBUTE void *msan_valloc(uptr size, StackTrace *stack);
SANITIZER_INTERFACE_ATTRIBUTE void *msan_pvalloc(uptr size, StackTrace *stack);
SANITIZER_INTERFACE_ATTRIBUTE void *msan_aligned_alloc(uptr alignment, uptr size, StackTrace *stack);
SANITIZER_INTERFACE_ATTRIBUTE void *msan_memalign(uptr alignment, uptr size, StackTrace *stack);
SANITIZER_INTERFACE_ATTRIBUTE int msan_posix_memalign(void **memptr, uptr alignment, uptr size,
                        StackTrace *stack);

SANITIZER_INTERFACE_ATTRIBUTE void InstallTrapHandler();
SANITIZER_INTERFACE_ATTRIBUTE void InstallAtExitHandler();

SANITIZER_INTERFACE_ATTRIBUTE const char *GetStackOriginDescr(u32 id, uptr *pc);

SANITIZER_INTERFACE_ATTRIBUTE void EnterSymbolizer();
SANITIZER_INTERFACE_ATTRIBUTE void ExitSymbolizer();
SANITIZER_INTERFACE_ATTRIBUTE bool IsInSymbolizer();
...
SANITIZER_INTERFACE_ATTRIBUTE void PrintWarning(uptr pc, uptr bp);
SANITIZER_INTERFACE_ATTRIBUTE void PrintWarningWithOrigin(uptr pc, uptr bp, u32 origin);

// Unpoison first n function arguments.
SANITIZER_INTERFACE_ATTRIBUTE void UnpoisonParam(uptr n);
SANITIZER_INTERFACE_ATTRIBUTE void UnpoisonThreadLocalState();

// Returns a "chained" origin id, pointing to the given stack trace followed by
// the previous origin id.
SANITIZER_INTERFACE_ATTRIBUTE u32 ChainOrigin(u32 id, StackTrace *stack);

SANITIZER_INTERFACE_ATTRIBUTE const int STACK_TRACE_TAG_POISON = StackTrace::TAG_CUSTOM + 1;
