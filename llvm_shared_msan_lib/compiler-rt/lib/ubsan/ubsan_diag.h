...
SANITIZER_INTERFACE_ATTRIBUTE SymbolizedStack *getSymbolizedLocation(uptr PC);
...
public:
  SANITIZER_INTERFACE_ATTRIBUTE Diag(Location Loc, DiagLevel Level, ErrorType ET, const char *Message)
      : Loc(Loc), Level(Level), ET(ET), Message(Message), NumArgs(0),
        NumRanges(0) {}
  SANITIZER_INTERFACE_ATTRIBUTE ~Diag();

  SANITIZER_INTERFACE_ATTRIBUTE Diag &operator<<(const char *Str) { return AddArg(Str); }
  SANITIZER_INTERFACE_ATTRIBUTE Diag &operator<<(TypeName TN) { return AddArg(TN); }
  SANITIZER_INTERFACE_ATTRIBUTE Diag &operator<<(unsigned long long V) { return AddArg(UIntMax(V)); }
  SANITIZER_INTERFACE_ATTRIBUTE Diag &operator<<(const void *V) { return AddArg(V); }
  SANITIZER_INTERFACE_ATTRIBUTE Diag &operator<<(const TypeDescriptor &V);
  SANITIZER_INTERFACE_ATTRIBUTE Diag &operator<<(const Value &V);
  SANITIZER_INTERFACE_ATTRIBUTE Diag &operator<<(const Range &R) { return AddRange(R); }
};
...
SANITIZER_INTERFACE_ATTRIBUTE bool ignoreReport(SourceLocation SLoc, ReportOptions Opts, ErrorType ET);
...
public:
  SANITIZER_INTERFACE_ATTRIBUTE ScopedReport(ReportOptions Opts, Location SummaryLoc, ErrorType Type);
  SANITIZER_INTERFACE_ATTRIBUTE ~ScopedReport();

  static void CheckLocked() { ScopedErrorReportLock::CheckLocked(); }
};

void InitializeSuppressions();
SANITIZER_INTERFACE_ATTRIBUTE bool IsVptrCheckSuppressed(const char *TypeName);
