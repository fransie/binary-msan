...
//
class JumpsPassDriver_t : public IRDB_SDK::TransformStep_t
{
	public:
		// 
		...
		int executeStep() override
		{
			...
			try
			{
				...
				auto success = JumpsPass(firp).execute();
	...
	private:
	// data
		const string program_name = string("jumps");	// constant program name
...
{
	return shared_ptr<TransformStep_t>(new JumpsPassDriver_t());
}
