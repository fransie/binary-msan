...
//
class TestPassDriver_t : public IRDB_SDK::TransformStep_t
{
	public:
		// 
		...
		int executeStep() override
		{
			...
			try
			{
				...
				auto success = TestPass(firp).execute();

				// check for success
				if (success)
				{
					cout << "Success! Thanos will DO SOMETHING HERE " <<  url << endl;
					return 0; // success (bash-style 0=success, 1=warnings, 2=errors)	
				}
	...
	private:
	// data
		const string program_name = string("test_pass");	// constant program name
...
{
	return shared_ptr<TransformStep_t>(new TestPassDriver_t());
}
