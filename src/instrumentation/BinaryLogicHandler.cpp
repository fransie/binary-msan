...
Instruction_t* BinaryLogicHandler::instrument(IRDB_SDK::Instruction_t *instruction) {
    std::cout << "BinaryLogicHandler: Instruction " << instruction->getDisassembly() << std::endl;
...
IRDB_SDK::Instruction_t* BinaryLogicHandler::instrumentRegRegInstruction(IRDB_SDK::Instruction_t *instruction) {
    ...
    if(dest == src && decInstr->getMnemonic() == "xor"){
        ...
        vector<basic_string<char>> instrumentationParams {to_string(dest), to_string(destWidth)};
